...
CliRunner.prototype = {
  ...
  initTestSettings : function(env, settings) {
    ...
    this.test_settings.debug = !!this.argv.debug;
    ...
    assert.ok(typeof this.test_settings.tag_filter === 'string');

    if (this.argv.tag) {
      assert.ok(typeof this.argv.tag === 'string');

      var tags = this.test_settings.tag_filter;

      if (tags && tags.length > 0) {

        // don't overwrite a tag_filter defined by an enviorment.
        // append new tags instead.
        tags = tags + ',' + this.argv.tag;
      } else {
        tags = this.argv.tag;
      }

      this.test_settings.tag_filter = tags;
    }

    this.test_settings.tag_filter = this.test_settings.tag_filter.split(',');

    if (typeof this.argv.skiptags == 'string') {
      this.test_settings.skiptags = this.argv.skiptags.split(',');
    }

    // expose the tag_filter, skiptags and debug to underlying test suites via an environment variable
    process.env.__NIGHTWATCH_TAGS = this.test_settings.tag_filter;
    process.env.__NIGHTWATCH_SKIPPED_TAGS = this.test_settings.skiptags;
    process.env.__NIGHTWATCH_DEBUG = this.test_settings.debug;
